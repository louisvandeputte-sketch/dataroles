...
class BrightDataLinkedInClient:
    ...
    async def trigger_collection_by_urls(self, urls: List[str]) -> str:
        ...
        Args:
            urls: List of LinkedIn job URLs to fetch
        
        Returns:
            snapshot_id for polling
        """
        # Bright Data expects "url" field in input for URL-based scraping
        payload = {
            "input": [{"url": url} for url in urls]
        }
        
        try:
            logger.info(f"Triggering Bright Data collection for {len(urls)} URLs")
            ...
            logger.success(f"URL collection triggered: snapshot_id={snapshot_id}")
            return snapshot_id
            
        except httpx.TimeoutException as e:
            logger.error(f"Timeout triggering URL collection: {e}")
            raise BrightDataError(f"Bright Data API timeout: {e}")
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error triggering URL collection: {e.response.status_code} - {e.response.text}")
            if e.response.status_code == 429:
                raise QuotaExceededError("Rate limit exceeded")
            elif e.response.status_code == 402:
                raise QuotaExceededError("Subscription quota exceeded")
            ...
            logger.exception(f"Unexpected error triggering URL collection: {e}")
            raise BrightDataError(f"Failed to trigger URL collection: {e}")
